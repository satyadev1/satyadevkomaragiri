...
export function Footer({ scrollProgress = 0 }) {
  ...
  const isCleanExecutive = typeof document !== 'undefined'
    && document.documentElement.dataset.theme === 'clean-executive';
  return (
    <footer id="footer" className="py-28 px-6 content-overlay">
      ...
      >
        <div className="footer-oval-inner">
          <h3
            className={`font-display text-2xl md:text-3xl font-semibold mb-4 ${isHighlighted && !isCleanExecutive ? 'text-white' : ''}`}
            style={isHighlighted
              ? { color: isCleanExecutive ? 'var(--footer-text-strong)' : undefined }
              : { color: 'var(--footer-text-strong)' }}
          >
            Let's build something together
          </h3>
          <p
            className={`text-sm md:text-base mb-10 max-w-md mx-auto leading-relaxed ${isHighlighted && !isCleanExecutive ? 'text-white/85' : ''}`}
            style={isHighlighted
              ? { color: isCleanExecutive ? 'var(--footer-text-body)' : undefined }
          ...
          <div className="flex flex-wrap justify-center gap-6 md:gap-10 mb-8">
            <a
              ...
              className={`inline-flex items-center gap-2.5 transition-colors text-sm font-medium ${isHighlighted && !isCleanExecutive ? 'text-white hover:text-white/80' : 'hover:text-accent'}`}
              style={isHighlighted
                ? { color: isCleanExecutive ? 'var(--footer-text-link)' : undefined }
          ...
          <p
            className={`text-xs ${isHighlighted && !isCleanExecutive ? 'text-white/70' : ''}`}
            style={isHighlighted
